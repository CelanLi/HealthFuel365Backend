...
import ItemSimilarityMatrixSchema, {
  ItemSimilarityMatrixInterface,
...
export const findRecommendations = async (): Promise<
...
> => {
  ...
  const detail = await ProductDetailSchema.find({
    ...
    $or: [{ fatLevel: "low" }, { sugarLevel: "low" }, { saltLevel: "low" }],
    $nor: [
      {
        $and: [
          { fatLevel: "high" },
  ...
  return getRandomizedRecommendations(
...
export const getRecommendationListById = async (
  _id: string
): Promise<RecommendationList[] | null | undefined> => {
...
const getRandomizedRecommendations = (
  recommendationsList: ProductInterface[]
): ProductInterface[] => {
...
const calculateItemUserMatrix = async (
  purchaseRecords: PurchaseRecord[]
): Promise<ItemUserMatrix> => {
  ...
  for (let i = 0; i < itemIds.length; i++) {
    ...
    for (let j = 0; j < itemIds.length; j++) {
      ...
      const similarity = calculateCosineSimilarity(
        itemUserMatrix[itemId1],
        itemUserMatrix[itemId2]
      );
...
export const generateRecommendationList = async (
  _id: string
): Promise<void> => {
  ...
  const queryConditions = {
    $and: [{ productID: { $in: recommendedIds } }, profileConditions],
  };
  ...
  const recommendationList = validRecommendProducts.map((item) => ({
  ...
  while (recommendationList.length < 20 && validProfileProducts.length > 0) {
    ...
    const isDuplicate = recommendationList.some(
      (product) => product.productID === randomProduct.productID
    );
    if (isDuplicate) {
...
const getSimilarProducts = async (itemId: string) => {
  ...
  const sortedProducts = similarProducts.sort(
