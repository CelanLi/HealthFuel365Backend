...
export async function getShoppingCartList(req: Request, res: Response) {
  const shoppingCartID = req.query.id;
  const requestedShoppingCart = {
    shoppingCartID: "12234",
    itemPrice: "343",
    totalSaving: 13324,
    subTotal: 413,
    itemQuantity: 143,
  };
  return res.status(200).json(requestedShoppingCart);
  
  ShoppingCartSchema.findById(shoppingCartID)
    .exec()
    .then((shoppingCart) => {
      if (!shoppingCart) {
        return res.status(404).json({
